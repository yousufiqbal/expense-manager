<script>
  ...
  const addCategory = async () => {
    const response = await post('/settings/categories/expense/add-category', category)
    const body = await response.json()
    if (response.ok) {
      addToast({ message: body.message })
      goto('/settings/categories/expense')
    }
  }
  
  const editCategory = async () => {
    const response = await put('/settings/categories/expense/edit-category?category-id=' + $page.url.searchParams.get('category-id'), category)
    const body = await response.json()
    if (response.ok) {
      addToast({ message: body.message })
      goto('/settings/categories/expense')
...
<Title title="{capitalize($page.params.mode)} Expense Category" back href="/settings/categories/expense" />
